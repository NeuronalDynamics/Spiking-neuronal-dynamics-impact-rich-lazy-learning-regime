...
from models.pl_modules_2_2 import add_structure_args, add_general_model_args, add_training_args, GLIFRN, RNN, LSTMN
...
def spectral_analysis(weight_matrix):
    ...
    return np.max(np.abs(eigenvalues.detach().numpy())) #normalized_spectrum
'''
...
def train(args, version): 
    ...
    for Sigma in range(Sigma_init, Sigma_end, Sigma_inc): #tau sigma_v R I_0
        ...
        if args.model in ["glifr_lheta", "glifr_fheta", "glifr_rheta", "glifr_homa", "glifr_lhet", "glifr_fhet", "glifr_rhet", "glifr_hom"]:
            outputs_0, _, _, _, activity_0 = model(inputs_0) #model.forward(inputs_0, track=True) #
        ...
        if args.model in ["glifr_lheta", "glifr_fheta", "glifr_rheta", "glifr_homa", "glifr_lhet", "glifr_fhet", "glifr_rhet", "glifr_hom"]:
            outputs_f, _, _, _, activity_f = model(inputs_0)#model.forward(inputs_0, track=True) #
        ...
        print(a_j_alignment)
        print("#########################################################")
        spectral_similarity = (final_spectrum - initial_spectrum) / (final_spectrum + initial_spectrum)
        ...
        print("#########################################################")
        output_weight_alignment = torch.norm(output_weight_f - output_weight_0)
        print(output_weight_alignment)
        print("#########################################################")
        weight_matrix_alignment = torch.norm(weight_matrix_f - weight_matrix_0)
        ...
        print("#########################################################")
        O_0 = torch.mm(outputs_0[:,-1,:].T, outputs_0[:,-1,:])
        O_t = torch.mm(outputs_f[:,-1,:].T, outputs_f[:,-1,:])
        # Calculate representation alignment
        representation_alignment_2 = torch.trace(torch.mm(O_t, O_0)) / (torch.norm(O_t, p='fro') * torch.norm(O_0, p='fro'))
        ...
        representation_alignment_log_2.append(representation_alignment_2.detach().numpy())
    ...
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_RA_1.csv", representation_alignment_log_1, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_RA_2.csv", representation_alignment_log_2, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_WMA.csv", weight_matrix_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_OWA.csv", output_weight_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_TTARA.csv", transform_to_asc_r_alignment_log, delimiter=",")
    ...
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_TKJA.csv", trans_k_j_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_TRJA.csv", trans_r_j_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_AJA.csv", a_j_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_TA.csv", thresh_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/{args.model}_{version}_TKMA.csv", trans_k_m_alignment_log, delimiter=",")
