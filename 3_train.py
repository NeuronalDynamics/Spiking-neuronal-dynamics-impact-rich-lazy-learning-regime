...
def train(args, version): 
    ...
    R_init = 1
    R_inc = 4
    R_end = R_init+(13*R_inc)
    
    for R in range(R_init, R_end, R_inc): #tau sigma_v R I_0
        # Analysis params 
        args.__dict__["tau"] = 0.5 #0.5
        args.__dict__["sigma_v"] = 1 #1
        args.__dict__["R"] = R / 100.0 #0.1
        args.__dict__["I_0"] = 0 #0
        print("********************************************************")
        print("R = ",args.R)
        print("********************************************************")
    ...
    np.savetxt(f"./analysis/R/{args.model}_{version}_R.csv", R_log, delimiter=",")
    np.savetxt(f"./analysis/R/{args.model}_{version}_TKA.csv", tangent_kernel_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/R/{args.model}_{version}_SS_1.csv", sign_similarity_log_1, delimiter=",")
    #np.savetxt(f"./analysis/R/{args.model}_{version}_SS_2.csv", sign_similarity_log_2, delimiter=",")
    np.savetxt(f"./analysis/R/{args.model}_{version}_RA.csv", representation_alignment_log_1, delimiter=",")
