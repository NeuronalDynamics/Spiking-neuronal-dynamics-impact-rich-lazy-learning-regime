...
def train(args, version): 
    ...
    V_reset_init = 0
    V_reset_inc = 1
    V_reset_end = V_reset_init+(13*V_reset_inc)
    
    for V_reset in range(V_reset_init, V_reset_end, V_reset_inc): #tau sigma_v R I_0
        # Analysis params 
        args.__dict__["tau"] = 0.5 #0.5
        args.__dict__["sigma_v"] = 1 #1
        args.__dict__["R"] = 0.1
        args.__dict__["I_0"] = 0
        args.__dict__["V_reset"] = V_reset / 100
        print("********************************************************")
        print("V_reset = ",args.V_reset)
        print("********************************************************")
    ...
    np.savetxt(f"./analysis/v_reset/{args.model}_{version}_V_reset.csv", V_reset_log, delimiter=",")
    np.savetxt(f"./analysis/v_reset/{args.model}_{version}_TKA.csv", tangent_kernel_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/v_reset/{args.model}_{version}_SS_1.csv", sign_similarity_log_1, delimiter=",")
    #np.savetxt(f"./analysis/I_0/{args.model}_{version}_SS_2.csv", sign_similarity_log_2, delimiter=",")
    np.savetxt(f"./analysis/v_reset/{args.model}_{version}_RA.csv", representation_alignment_log_1, delimiter=",")
