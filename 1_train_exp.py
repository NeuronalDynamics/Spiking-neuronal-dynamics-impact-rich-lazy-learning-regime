...
def compute_ntk(model, input):
    # Ensure model is in evaluation mode to disable any dropout or batch normalization
    model.eval()

    # Function to compute the model's output (we assume the model's forward function returns the output directly)
    def model_output(x):
        out, _, _, _, hidden = model(x)
        hidden_activity_last_step = hidden[:,-1,:]
        return out

    # Batch the computation of the Jacobian across the batch dimension
    jacobian = torch.func.jacfwd(model_output)(input)
    print(input.shape)
    #print(model.parameters())
    #for i, tensor in enumerate(tuple(model.parameters())):
    #    print(f"Shape of tensor {i}: {tensor.shape}")
    for i, tensor in enumerate(jacobian):
        print(f"Shape of tensor {i}: {tensor.shape}")
    j = torch.func.vmap(jacobian)

    # Compute the Neural Tangent Kernel by taking the product of the Jacobian with its transpose
    # We sum over the parameter and input dimensions to get the final NTK matrix
    ntk = torch.einsum('ij,kj->ik', jacobian, jacobian)
...
def compute_ntk(net, x):
    ...
    # Initialize an empty list to hold the gradients (rows of the Jacobian matrix)
    jacobian = []

    for i in range(y.size(0)):  # Assuming y is already the output of the model
        # Clear existing gradients
        if x.grad is not None:
            x.grad.zero_()
        
        # Create a tensor of zeros with the same shape as y and set 1 at the ith position
        # This tensor serves as the 'grad_outputs' argument
        grad_outputs = torch.zeros_like(y)
        grad_outputs[i] = 1
        
        # Compute gradient of y[i] with respect to x  #tuple(net.parameters())
        grad = torch.autograd.grad(outputs=y, inputs=x, grad_outputs=grad_outputs, retain_graph=True, allow_unused=True )[0]
        grad = torch.cat([grad.flatten()])
        jacobian.append(grad)

    # Stack the gradients to form the Jacobian matrix
    jacobian_matrix = torch.stack(jacobian, dim=0)

    #print(jacobian_matrix.shape)
    #nkt = jacobian_matrix @ jacobian_matrix.T
    ntk = torch.einsum('ij,kj->ik', jacobian_matrix, jacobian_matrix)
...
if __name__ == '__main__':
    ...
    for i in range(args.ntrials):
        ...
        ckpt_dict = {}
        to_train = ["glifr_lheta"] #glifr_fheta glifr_rheta glifr_homa
