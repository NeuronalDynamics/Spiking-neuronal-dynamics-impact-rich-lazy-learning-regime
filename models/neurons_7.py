...
class GLIFR(nn.Module):
        ...
        def hom_init_(self):
                with torch.no_grad():
                        ...
                        nn.init.uniform_(self.weight_iv, -math.sqrt(0.6 / self.hidden_size), math.sqrt(0.6 / self.hidden_size))
                        nn.init.uniform_(self.weight_lat, -math.sqrt(0.6 / self.hidden_size), math.sqrt(0.6 / self.hidden_size))
...
class RNNC(nn.Module): 
        ...
        def forward(self, x, hidden, hidden_delayed, track = True):
                """
