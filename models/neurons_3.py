...
class SurrogateSpikeFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, threshold, sigma_v):
        # Ensure threshold and sigma_v are tensors before saving
        if not isinstance(threshold, torch.Tensor):
            threshold = torch.tensor(threshold, device=input.device, dtype=input.dtype)
        if not isinstance(sigma_v, torch.Tensor):
            sigma_v = torch.tensor(sigma_v, device=input.device, dtype=input.dtype)
        
        ctx.save_for_backward(input, threshold, sigma_v)
        out = torch.sigmoid((input - threshold) / 0.001)
        return out

    @staticmethod
    def backward(ctx, grad_output):
        input, threshold, sigma_v = ctx.saved_tensors
        # Surrogate gradient: derivative of fast sigmoid approximation
        grad_input = grad_output * (torch.sigmoid((input - threshold) / sigma_v) * (1 - torch.sigmoid((input - threshold) / sigma_v)))
        return grad_input, None, None  # Return None for gradients w.r.t. threshold and sigma_v as they're not involved here
...
class GLIFR(nn.Module):
        ...
        def spike_fn(self, x):
                ...
                return SurrogateSpikeFunction.apply(x, self.thresh, self.sigma_v) #torch.sigmoid(activation)
        
        def spike_fn_derivative(self, x):
                """Derivative of the spiking activation function."""
                sigmoid = SurrogateSpikeFunction.apply(x, self.thresh, self.sigma_v)
                return sigmoid * (1 - sigmoid) / self.sigma_v
