...
from models.neurons_4 import GLIFR, RNNC, NeuronParameters, StructureParameters
...
class BaseModule(pl.LightningModule):
        ...
        def training_epoch_end(self, outputs):
                loss_values = [x['loss'].item() for x in outputs]
                if loss_values[-1] < self.loss_stop_criteria:
                    self.trainer.should_stop = True

        def test_epoch_end(self, outputs):
                acc_value = [x['acc'].item() for x in outputs]
                loss_values = [x['loss'].item() for x in outputs]

                current_datetime = datetime.now()
                formatted_datetime = current_datetime.strftime('%Y%m%d%H%M%S')

                np.savetxt(f"./analysis/sigma_v/test_acc_{formatted_datetime}.csv", acc_value, delimiter=",")
                np.savetxt(f"./analysis/sigma_v/test_loss_{formatted_datetime}.csv", loss_values, delimiter=",")
        ...
        def test_step(self, batch, batch_idx):
                ...
                acc = (y_hat_disc.cpu().detach().data == y_disc.cpu()).sum() / len(y)
                if self.hparams.log_accuracy:
                        self.log(f"test_acc", acc, on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)

                self.log(f"test_loss", loss.cpu().item(), on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
                return {"loss": loss, "acc": acc}

        '''
        def configure_optimizers(self):
            # setup your optimizer and learning rate scheduler here
            optimizer = torch.optim.Adam(self.parameters(), lr=self.hparams.lr)
            return optimizer

        def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx,
                           optimizer_closure, on_tpu, using_native_amp, using_lbfgs):
            # calling optimizer step function
            optimizer.step(closure=optimizer_closure)

            # perform gradient clipping
            torch.nn.utils.clip_grad_norm_(self.parameters(), max_norm=0.6)
...
class GLIFRN(BaseModule):
        ...
        def forward(self, input, track=True):
            """
            ...
            if track:
                ...
                firings = torch.empty((self.batch_size, nsteps, self.hparams.hidden_size)).type_as(input)  # To track firing rates
            ...
            for step in range(nsteps):
                ...
                #########################################################################################
                hidden_layer_output_clone = self.firing.clone()
                #########################################################################################
                ...
                if track:
                    ...
                    firings[:, step, :] = hidden_layer_output_clone#self.firing.clone()#
                
                if len(outputs_) > delay + 1:
                    outputs_ = outputs_[-delay-1:]
            # Return
            if track:
                    return outputs, voltages, ascs, syns, firings
            else:
                    return outputs
