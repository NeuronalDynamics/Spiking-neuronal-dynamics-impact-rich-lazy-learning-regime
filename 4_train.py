...
def train(args, version): 
    ...
    I_0_init = 0
    I_0_inc = 1
    I_0_end = I_0_init+(13*I_0_inc)
    
    for I_0 in range(I_0_init, I_0_end, I_0_inc): #tau sigma_v R I_0
        # Analysis params 
        args.__dict__["tau"] = 0.5 #0.5
        args.__dict__["sigma_v"] = 1 #1
        args.__dict__["R"] = 0.1
        args.__dict__["I_0"] = I_0 / 100 #0
        print("********************************************************")
        print("I_0 = ",args.I_0)
        print("********************************************************")
    ...
    np.savetxt(f"./analysis/I_0/{args.model}_{version}_I_0.csv", I_0_log, delimiter=",")
    np.savetxt(f"./analysis/I_0/{args.model}_{version}_TKA.csv", tangent_kernel_alignment_log, delimiter=",")
    np.savetxt(f"./analysis/I_0/{args.model}_{version}_SS_1.csv", sign_similarity_log_1, delimiter=",")
    #np.savetxt(f"./analysis/I_0/{args.model}_{version}_SS_2.csv", sign_similarity_log_2, delimiter=",")
    np.savetxt(f"./analysis/I_0/{args.model}_{version}_RA.csv", representation_alignment_log_1, delimiter=",")
