...
from datasets.neurogym import NeurogymDataModule
from datasets.utils import add_data_args
from models.pl_modules_7 import add_structure_args, add_general_model_args, add_training_args, GLIFRN, RNN, LSTMN
...
def train(args, version): 
    ...
    Sigma_init = 1
    Sigma_inc = 1
    Sigma_end = Sigma_init+(10*Sigma_inc)
    ...
    for Sigma in range(Sigma_init, Sigma_end, Sigma_inc): #tau sigma_v R I_0
        ...
        if args.model in ["glifr_lheta", "glifr_fheta", "glifr_rheta", "glifr_homa", "glifr_lhet", "glifr_fhet", "glifr_rhet", "glifr_hom"]:
            _, voltages_0, _, _, activity_0 = model(inputs_0)
        ...
        if args.model in ["glifr_lheta", "glifr_fheta", "glifr_rheta", "glifr_homa", "glifr_lhet", "glifr_fhet", "glifr_rhet", "glifr_hom"]:
            _, voltages_f, _, _, activity_f = model(inputs_0)
    ...
    np.savetxt(f"./analysis/sigma_v/38/{args.model}_{version}_Sigma.csv", Sigma_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/38/{args.model}_{version}_RA.csv", representation_alignment_log_1, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/38/{args.model}_{version}_WMC.csv", weight_matrix_change_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/38/{args.model}_{version}_EWCN.csv", effective_weight_change_norm_log, delimiter=",")
    np.savetxt(f"./analysis/sigma_v/38/{args.model}_{version}_JCN.csv", Jacobian_change_norm_log, delimiter=",")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--task", type=str, choices=["sine", "nmnist", "pmnist", "lmnist", "neurogym"])
