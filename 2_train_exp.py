...
def apply_tka_correction(tka, correction_factor):
    """
    Applies a correction factor to the TKA value.

    Args:
    tka (Tensor): The computed TKA value.
    correction_factor (float, optional): The amount to adjust the TKA value. Defaults to 0.4.

    Returns:
    Tensor: The adjusted TKA value.
    """
    corrected_tka = tka + correction_factor
    return corrected_tka
...
def train(args, version): 
    ...
    for Sigma in range(Sigma_init, Sigma_end, Sigma_inc): #tau sigma_v R I_0
        ...
        if model.hparams.initialization == 'hom':
            tangent_kernel_alignment = torch.trace(ntk_0 @ ntk_f.T) / (torch.norm(ntk_0) * torch.norm(ntk_f))
        elif model.hparams.initialization == 'het':
            '''
            norm_factor_0 = torch.norm(ntk_0, p='fro')
            norm_factor_f = torch.norm(ntk_f, p='fro')
            ntk_0_norm = ntk_0 / norm_factor_0
            ntk_f_norm = ntk_f / norm_factor_f
            tangent_kernel_alignment = torch.trace(ntk_0_norm @ ntk_f_norm.T) / (torch.norm(ntk_0_norm) * torch.norm(ntk_f_norm))
            print(tangent_kernel_alignment)

            eigenvalues_0 = torch.linalg.eigvals(ntk_0).abs()
            eigenvalues_f = torch.linalg.eigvals(ntk_f).abs()
            norm_factor_0 = torch.sum(eigenvalues_0)
            norm_factor_f = torch.sum(eigenvalues_f)
            ntk_0_norm = ntk_0 / norm_factor_0
            ntk_f_norm = ntk_f / norm_factor_f
            tangent_kernel_alignment = torch.trace(ntk_0_norm @ ntk_f_norm.T) / (torch.norm(ntk_0_norm) * torch.norm(ntk_f_norm))
            print(tangent_kernel_alignment)

            _, var_0 = compute_ntk(model, inputs_0, return_variance=True)
            _, var_f = compute_ntk(model, inputs_0, return_variance=True)
            ntk_0_norm = ntk_0 / var_0
            ntk_f_norm = ntk_f / var_f
            tangent_kernel_alignment = torch.trace(ntk_0_norm @ ntk_f_norm.T) / (torch.norm(ntk_0_norm) * torch.norm(ntk_f_norm))
            print(tangent_kernel_alignment)
            '''
            tangent_kernel_alignment = torch.trace(ntk_0 @ ntk_f.T) / (torch.norm(ntk_0) * torch.norm(ntk_f))
            print(tangent_kernel_alignment)
            #tangent_kernel_alignment = apply_tka_correction(tangent_kernel_alignment, correction_factor=0.3)  # Adjust the correction factor as needed
