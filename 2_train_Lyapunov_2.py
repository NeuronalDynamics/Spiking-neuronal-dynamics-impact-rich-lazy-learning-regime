...
from models.pl_modules_6_2 import add_structure_args, add_general_model_args, add_training_args, GLIFRN, RNN, LSTMN
from training.callbacks import *
from utils.types import NeuronParameters
from pytorch_lightning.callbacks import EarlyStopping
import sys
from lyapunov.LEs_2 import calculateLEs, jacobian_eigenspectrum
...
def train(args, version): 
    ...
    for Sigma in range(Sigma_init, Sigma_end, Sigma_inc): #tau sigma_v R I_0
        ...
        np.savetxt(f"./analysis/lyapunov/{args.model}_{version}_sigma_v=_{args.sigma_v}_LEs.csv", les, delimiter=",")
        np.savetxt(f"./analysis/lyapunov/{args.model}_{version}_sigma_v=_{args.sigma_v}_jacobian_norms.csv", jacobian_norms, delimiter=",")
        np.savetxt(f"./analysis/lyapunov/{args.model}_{version}_sigma_v=_{args.sigma_v}_max_eigenvalues.csv", max_eigenvalues, delimiter=",")
        np.savetxt(f"./analysis/lyapunov/{args.model}_{version}_sigma_v=_{args.sigma_v}_derivative_history.csv", derivative_history, delimiter=",")
