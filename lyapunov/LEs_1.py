...
def calculateLEs(model, num_steps=100):
    batch_size = 1
    input_sequence = torch.randn(batch_size, num_steps, 2, dtype=torch.float64)
    firing, voltage, ascurrent, syncurrent = model.init_states(batch_size) #252 #2
    #h_0, _, _, _ = model.init_states(batch_size=2) #252 #2
    #print("h_0", h_0.shape)
    #torch.Size([252, 252]) 
    #(batch_size, hidden_size)
    Q = torch.eye(firing.size(dim=1), dtype=torch.float64) 
    #print("Q", Q)

    lyapunov_exponents_sum = torch.zeros((batch_size, model.hidden_size))
    jacobian_norms = []
    max_eigenvalues = []

    for t in range(num_steps):
        firing, voltage, ascurrent, syncurrent = model.forward(input_sequence[:, t, :], firing, voltage, ascurrent, syncurrent)
        J = model.spike_fn_derivative(firing)

        # Calculate Frobenius norm
        frob_norm = torch.norm(J, p='fro').item()
        jacobian_norms.append(frob_norm)
        #print(frob_norm)
